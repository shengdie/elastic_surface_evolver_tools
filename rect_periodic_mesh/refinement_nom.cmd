...
procedure refinemarked(){
    ...
    foreach vertex vv where old_vid == 0 do {
        ...
        //};
        //print "set refine";
        if ! (vv.on_boundary 1 || vv.on_boundary 2) then {
            set vv bend; /* set vv constraint 1; set vv constraint 2; */ /*set vv gbend; */

            if valid_constraint(1) then {
                if edrgap then {
                    if vv.ref_coord[1] == lngth || vv.ref_coord[1] == 0 then set vv constraint 1;
                } else {
                    set vv constraint 1;
                };
            };
            //if is_defined("egap") then  {
            //    if edgegap then {
            //        if vv.ref_coord[1] == lngth then set vv egap;
            //    } else {
            //        set vv egap;
            //    };
            //};
