...
outsurfdict := {
    ...
    //printf "# Surface output: vertices, faces, thickness, gridsize, total_energy, refine_times, poisson_ratio, period or tension\n";
    printf "{";
    printf "\"nv\": %g, \"nf\": %g, \"thickness\": %g, \"gridsize\": %0.15g, \"energy\": %0.15g, \"refine_times\": %g, \"pratio\": %g, \"period\": %0.15g, ",
        vertex_count, facet_count, thicknezz, gridsize, total_energy, refine_times, facets[1].poisson_ratio, upEndY;
    printf "\"sube\": %0.15g, \"stretche\": %0.15g, \"bende\": %0.15g, \"min_wavnum\": %g, \"max_wavnum\": %g, ", subenergy.value, stretch.value, bend.value, num0, numl;
    ...
    if is_defined("wd") then exec "printf \"\\\"_w\\\": %0.15g, \", wd";
    if is_defined("lngth") then exec "printf \"\\\"_l\\\": %0.15g, \", lngth";
...
save_surf := { exec sprintf "outputsurf | \"cat > %s_h%g_min%g_max%g_r%g_$(date +%%Y_%%m_%%d_%%H_%%M_%%S).out\"", origin_name,thicknezz/wd,num0, numl,refine_times }
save_dict := { exec sprintf "outsurfdict | \"cat > %s_h%g_t%g_min%g_max%g_r%g_$(date +%%Y_%%m_%%d_%%H_%%M_%%S).out\"", origin_name,thicknezz/wd,tensionG,num0, numl,refine_times }


/* used some trick to dump the surface to the {datafilename}_{YYYY_mm_dd_HH_MM_SS} */

dump_surf :=
  { exec sprintf "print \"\" | \"echo dump \\\\\\\"%s_h%g_t%g_min%g_max%g_r%g_$(date +\\\"%%Y_%%m_%%d_%%H_%%M_%%S\\\").dmp\\\\\\\" > dump.cmd; sleep 1\"",origin_name,thicknezz/wd,tensionG,num0, numl,refine_times;
    read "dump.cmd";
    print "" | "rm dump.cmd";
  }
...
procedure mark_curvature(real ceng) {
  foreach vertex vv where vv.bend > ceng do set vv.edge color red;
}

procedure refine_curvature(real ceng) {
  foreach vertex vv where vv.bend > ceng do set vv.edge divide 1;
  foreach facet ff do {
    if sum(ff.edge, divide) > 1.8 then set ff.edge divide 1;
  };
  refinemarked();
}
